...
async def get_drift_history(model_id: str, hours: int = Query(24, ge=1, le=168)) -> APIResponse:
    """
    ...
    return APIResponse(
        ...
        data={
            ...
            "analysis": {
                "trend": (
                    "increasing"
                    if history[-1]["data_drift"] > history[0]["data_drift"]
                    else "stable"
                ),
    ...
    return APIResponse(
        ...
        data={
            ...
            "threats_24h": {
                ...
                "other": {"total": 5 + random.randint(-2, 2), "blocked": 4 + random.randint(-1, 1)},
            },
            "auth_metrics": {
...
async def get_live_metrics() -> APIResponse:
    ...
    return APIResponse(
        ...
        data={
            ...
            "models": {"active": 12, "healthy": 10, "degraded": 2, "unhealthy": 0},
...
async def get_live_timeseries(
    ...
    return APIResponse(
        success=True, data={"metric": metric, "points": data, "resolution_seconds": 10}
    )
